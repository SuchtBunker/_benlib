...
function AddEntity( ent )
	...
	ent["CurrentUndo"] = Current_Undo
	ent["CurrentUndoEntId"] = table.insert( Current_Undo.Entities, ent )
...
function ReplaceEntity( from, to )
	local tab = from["CurrentUndo"]
	if !tab then return false end

	local id = from["CurrentUndoEntId"]
	tab["Entities"][id] = to

	if IsValid(to) then
		to["CurrentUndo"] = tab
		to["CurrentUndoEntId"] = id
	end
	from["CurrentUndo"] = nil
	from["CurrentUndoEntId"] = nil
