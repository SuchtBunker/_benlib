...
if ( SERVER ) then
	...
	function Add( pl, type, ent )
		...
		ent["CurrentCleanup"] = cleanup_list[ id ][ type ]
		ent["CurrentCleanupEntId"] = table.insert( cleanup_list[ id ][ type ], ent )
	end

	function ReplaceEntity( from, to )
		local tab = from["CurrentCleanup"]
		if !tab then return false end

		local id = from["CurrentCleanupEntId"]
		tab[id] = to

		if IsValid(to) then
			to["CurrentCleanup"] = tab
			to["CurrentCleanupEntId"] = id
		end
		from["CurrentCleanup"] = nil
		from["CurrentCleanupEntId"] = nil
